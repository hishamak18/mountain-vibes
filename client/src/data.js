 export const RoomImg=[
    {
        img:"https://flic.kr/p/2otKdLq",
        tittle:"Single Room"
        ,price:"1500INR/DAY"
    }
    ,{
        img:"https://images.pexels.com/photos/271624/pexels-photo-271624.jpeg?auto=compress&cs=tinysrgb&w=1600"
        ,tittle:"Family Room"
        ,price:"1500INR/DAY"
    }
    ,{
        img:"https://images.pexels.com/photos/271619/pexels-photo-271619.jpeg?auto=compress&cs=tinysrgb&w=1600"
        ,tittle:"Presidential Room"
        ,price:"1500INR/DAY"
    }
]

export const images = [

    { src: 'https://flic.kr/p/2otN9Sg', alt: 'Image 4' },
     { src: 'https://flic.kr/p/2otKedY', alt: 'Image 4' },
     { src: 'https://flic.kr/p/2otQoVE', alt: 'Image 4' },
     { src: 'https://flic.kr/p/2otKdLq', alt: 'Image 2' },
     { src: 'https://flic.kr/p/2otKdLR', alt: 'Image 2' },

    
     { src: 'https://flic.kr/p/2otP43B', alt: 'Image 2' },
    { src: 'https://flic.kr/p/2otQedv', alt: 'Image 3' },
    { src: 'https://flic.kr/p/2otQeoR', alt: 'Image 4' },
    { src: 'https://flic.kr/p/2otP42z, alt: 'Image 4' },
  ];
  
